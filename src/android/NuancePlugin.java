...
/**
 * Plugin to use Nuance Speech Recognition
 * @author fniwes based on plugin from asmyth
 *
 */
public class NuancePlugin extends CordovaPlugin {
	...
	public boolean execute(String action, JSONArray data, CallbackContext callbackContext) throws JSONException {
		...
		try{
			...
			if (ACTION_INIT.equals(action)) { // INITALIZE
				...
				Log.e("NuancePlugin", "NuancePlugin.execute: Invalid damned action ["+action+"] passed");
			}
	...
	private PluginResult initSpeechKit(JSONArray data, CallbackContext callbackContext) throws JSONException{
		...
		String serverName = data.getString(0);
		int port = data.getInt(1);
		boolean sslEnabled = data.getBoolean(2);
		String appId = data.getString(3);
		String secret = data.getString(4);

		Log.d("NuancePlugin", "Init: Server = ["+serverName+"]");
		Log.d("NuancePlugin", "Init: Port = ["+port+"]");
		Log.d("NuancePlugin", "Init: SSL = ["+sslEnabled+"]");
		Log.d("NuancePlugin", "Init: App Id = [" + appId + "]");
		
		JSONObject returnObject = new JSONObject();
		
		try{
	        if (speechKit == null){
	        	byte[] appKey = new CredentialsParser(secret).get();
